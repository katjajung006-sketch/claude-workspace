...
Die Säulen des Gewinner-Musters (alle Pflicht): (0) **Herzstück — eine durchgehende Körper-Metapher**: die Übung verkörpert körperlich genau das Gefühl aus dem Hook (Original: „alles zusammenhalten" ↔ die zusammengehaltenen Knie fallen auf). Hook, Übung und Caption-Reframe sagen dasselbe, einmal im Körper, einmal im Gefühl — DAS ist der Motor, nicht der Hook allein. · (1) **Insider-/Advocacy-Hook** („Ich wünschte, mehr Frauen wüssten, was diese 3 Minuten machen, wenn du den ganzen Tag alles zusammengehalten hast") · (2) **Erkennungs-Caption** („Die meisten Frauen…") + Reframe („…der erste Moment, in dem du nichts zusammenhalten musst") · (3) Versprechen gespürt-körperlich, nie Leistung · (4) Übung super einfach + bodennah (Original = **liegender Schmetterling**, NICHT liegende Drehung) · (5) **Speicher- + Kommentar-Treiber fest eingebaut** (der eigentliche Reichweiten-Hebel — der Gewinner hatte 127 Speicherungen, 220 Reaktionen, 15 Kommentare → +2,9x Distribution). Abgrenzung zu `/reel-yoga-wirkung`: das ist das *offene* Wirkungs-Format zum Hook-Testen; `/reel-gewinner` ist die *enge, datenbelegte* Variante, immer auf Speichern/Kommentar/Distribution optimiert. Output wie `/reel-yoga-wirkung` plus 2 Alt-Hooks. (Name vorläufig — kann später umbenannt werden. Datenquelle: Memory `project_reel_gewinner_format`.)
...
Nutzt `reference/persoenlicher-content.md` als Single Source of Truth (Feed/Story-Trennung, Leitplanken, CTA-Logik) + `context/founder-story.md` als Inhaltsquelle; Format-Gerüst aus `reel.md`/Karussell-Skills — kein Doppeln. Pflicht-Regeln: Hook **als Zeile 1 kennzeichnen** + Härtetest (echter Scroll-Stopper, kein leiser Alltagssatz) — gilt auch für die 2 Alt-Hooks, Form rotieren; **zwei Format-Varianten** je Reel (Text im Reel + Voiceover mit Untertiteln, Katja wählt nach Zeit); **Katja ist raus** aus dem Funktionsmodus (nie „noch mittendrin/noch nicht raus") und **im Bild** (nie „faceless"); jeder Post endet bei der Soul-Client, nie Tagebuch; CTA bewusst (default leiser Folgen-Grund, kein reflexhaftes Speichern, Freebie nur ausnahmsweise); Output inkl. 2 Alt-Hooks zum Tauschen. Notion-Ablage (nur auf Abnahme) → DB „Feed", Content-Säule „Persönlich".
