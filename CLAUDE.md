...
Wirkung jeder Haltung wird vorher geprüft (wahr, in Katjas Sprache, ohne Fachbegriffe). Pflicht-Schritt Situations-Anker: jedes Reel hängt an EINEM konkreten Soulclient-Moment — trägt Hook, Cover und Weiterleitungs-Wirkung. Kein Caption-Verweis im Video, CTAs nur in der Caption. Freebie-CTA: je Reel das passende der vier Freebies wählen und über Pieces hinweg abwechseln — **3MINUTEN** (3-Minuten-Körpercheck) bei kurzem Übungs-/Spür-Fokus tagsüber · **RESET** (5-Minuten-Körper-Reset) bei Abend-/Feierabend-/Abschalten-Fokus · **NACHTRUHE** (Einschlaf-Freebie „Wenn der Kopf nicht ausmacht") bei Einschlaf-/im-Bett-/„Kopf macht nicht aus"-Fokus · **CHECK** (Funktionsmodus-Check) bei Erkennen-/Funktionsmodus-Fokus; nie zwei Keywords in einem Post mischen (ManyChat-Flow). Alle vier Auto-DM-Flows sind angelegt (CHECK live; 3MINUTEN, RESET, NACHTRUHE als Draft gebaut 2026-06-23) — RESET/NACHTRUHE gehen am 28.06. mit echtem Trigger-Reel live.
...
**Wichtig — NICHT aufs Schlafen verengen** (Katja-Klärung 2026-06-27): Es geht um die **Stille und was der Kopf dann macht**, nicht ums Einschlafen. Die Nacht ist nur EIN stiller Moment (genauso: Kinder schlafen, im Auto, abends hinsetzen) — Moment rotieren. Säulen (alle Pflicht): (0) **Herzstück — über den Körper an den Kopf:** die Haltung gibt dem Dauer-Standby-Körper das Signal „du darfst loslassen"; die Caption-Auflösung führt über Körper-Empfindungen (gehalten, schwer, weich), der Kopf folgt ZULETZT („und irgendwann macht auch der Kopf langsam mit"). Das ist der Motor, nicht der Hook allein. · (1) **Erkennungs-Hook am stillen Moment** („Es ist still. Aber dein Kopf macht nicht aus.") · (2) **Reframe** („Es ist nicht [X], es ist dein Kopf, der nicht aufgehört hat" — nimmt die Schuld) · (3) **konkreter körperlicher Anker, direkt im Video gezeigt** (Übung sofort sichtbar, kein offener Loop) · (4) stiller Moment variabel. Freebie je Moment: Nacht/Bett → NACHTRUHE · Abend → RESET · Erkennen → CHECK · kurze Übung tagsüber → 3MINUTEN. Output wie `/reel-yoga-wirkung` plus 2 Alt-Hooks. (Name vorläufig. Datenquelle: Memory `project_reel_gewinner_format`.)
...
Nutzt `reference/persoenlicher-content.md` als Single Source of Truth (Feed/Story-Trennung, Leitplanken, CTA-Logik) + `context/founder-story.md` als Inhaltsquelle; Format-Gerüst aus `reel.md`/Karussell-Skills — kein Doppeln. Pflicht-Regeln: Hook **als Zeile 1 kennzeichnen** + Härtetest (echter Scroll-Stopper, kein leiser Alltagssatz) — gilt auch für die 2 Alt-Hooks, Form rotieren; **nur Voiceover im Poetry-Format** (kein Text-im-Reel mehr, Katja-Entscheidung 2026-07-01); **Katja ist raus** aus dem Funktionsmodus (nie „noch mittendrin/noch nicht raus") und **im Bild** (nie „faceless"); jeder Post endet bei der Soul-Client, nie Tagebuch; CTA bewusst (default leiser Folgen-Grund, kein reflexhaftes Speichern, Freebie nur ausnahmsweise); Output inkl. 2 Alt-Hooks zum Tauschen. Notion-Ablage (nur auf Abnahme) → DB „Feed", Content-Säule „Persönlich".
...
Kein starres Mo-So-Raster mehr: **alles rotiert von Woche zu Woche** über `outputs/story-woche/verlauf.md` — Tag→Format-Zuordnung (Woche startet nie zweimal mit demselben Format), welche 5 Verkaufsformate, der Inhalt jeder Vorlage, Angebot/Keyword (alle 4 Freebies + Produkt, kein Keyword 2×/Woche), Sticker, Tageszeit (morgens/mittendrin/abends — nicht nur abends), Winkel (aus `reference/zielgruppe-workbook.md`).

Eingebaute Regeln: max. 2 Verkaufstage am Stück · max. 1–2 harte Produkt-Pitches/Woche, nie nebeneinander · ein Tag = ein Ziel · Polls max. 2–3×/Woche · kurz halten, Bestes zuerst · abends 18–22 Uhr posten · Link-Disziplin (Freebie → Auto-DM-Keyword/Opt-in · Produkt „10-Minuten-Rückkehr" → `10-minuten-rueckkehr.katjajung.com`, **nie zur Kasse**, 7-€-Tripwire nie öffentlich) · Freebie-Keywords RESET/3MINUTEN/NACHTRUHE/7ZEICHEN erst ab 28.06. live (davor CHECK/RÜCKKEHR). Volle Logik + Vorlagen-Pool in `.claude/commands/story-woche.md`. Verkaufs-Ton ohne Hype, menschliche Schicht (6 Türen) natürlich eingewoben statt formelhaft auf alle 7 Tage.
...
- **Übungs-Datenbank** `reference/uebungs-datenbank.md` (angelegt 2026-06-28): 34 Übungen mit **von Katja validierten Wirkungen** („Was wirklich passiert"), Körper-Metapher, **Zielgruppen-Problem (→ Hook/Beat 1)**, Körper-Anker (Übungslogik), Moment und Freebie. **Bei jedem übungsbasierten Content zuerst hier nachschlagen** (Übung + echte Wirkung + Metapher rausziehen statt herleiten) — verankert im Schritt „Wirkung prüfen" aller Übungs-Skills. **Beat 1 / den Hook IMMER aus der Zielgruppen-Problem-Zeile ziehen** (ganze Sätze über ihr inneres Erleben, aus `soulclient.md`; ausgebaut 2026-07-01 — Übungen tragen mehrere Probleme, Überlappung ist erwünscht), NIE über den Körperteil der Übung führen. Steht eine Übung nicht drin: recherchieren UND Katja zur Wirkung fragen, dann eintragen (Wirkung kommt nur aus Katjas Hand). Details: Memory `project_uebungs_datenbank`.
...
**Vierte Datenbank — „Verkaufs-Stories"** (auf derselben Seite „yoga.statt.funktionieren", `data_source_id` `29fc89c3-46e1-4145-bcea-9fdce5244392`, angelegt 2026-06-27): Stories, die verkaufen — das Story-Secrets-System (Olga Weber) übersetzt in Katjas Tonalität. Spalten: Story (Titel) · Typ (Mythos aufdecken / Fallstudie / Persönliche Story / Behind the Scenes / Fragen & Antworten / Verkaufs-Story 1-teilig / Storylaunch 10 Tage) · Status · Verkauft (welches Angebot) · Keyword · Slides · CTA/Link-Sticker · Content-Säule · Reihenfolge · Notiz. Slide-Texte copy-fertig als Zitat-Blöcke im Seiteninhalt, **plus 2 Alt-Hooks je Eintrag** (Feed-Content-Logik). Grundlage & Ton-Regeln: `reference/stories-verkaufen.md` (das „Wie verkaufe ich") + `reference/zielgruppe-workbook.md` (das „Was löse ich / was will sie", Grundlage für ALLEN Content). Kurs-PDFs liegen in `reference/kurs-stories-verkaufen/`. Verkaufs-Stories immer in DIESE DB. Nur auf ausdrückliche Abnahme ablegen.
...
- `website-quelle/` — **Quelldateien**: `index.html` (Landingpage), `angebot.html` (€7-Tripwire-Verkaufsseite), `kurs.html` (öffentliche €17-Kursseite), `optin.html`, `start.html` (Instagram-Bio-Link-Hub), `katja.jpg`. Änderungen hier vornehmen. (Früher `netlify-deploy/` — umbenannt 2026-06-22, weil der alte Name verwirrte; es geht nichts zu Netlify, das ist nur der Quell-Ordner für den Cloudflare-Deploy.)
- `cloudflare-deploy/check/`, `cloudflare-deploy/kurs/` und `cloudflare-deploy/start/` — **Deploy-Ordner** für die drei Cloudflare-Pages-Projekte. Vor jedem Deploy aus `website-quelle/` synchronisieren.
...
Live-Domains: `check.katjajung.com` (Landingpage + €7-Seite), `10-minuten-rueckkehr.katjajung.com` (€17-Kursseite) und `start.katjajung.com` (**Bio-Link-Hub**, gebaut 2026-06-28, ersetzt Linktree als zweiten Instagram-Bio-Link; neue Miniprodukte einfach als Karte in `website-quelle/start.html` ergänzen). Bezahlung/Kurse/E-Mail-Marketing laufen weiterhin über **systeme.io** — Cloudflare ist nur die Anzeige-Hülle, die Kauf-Buttons verlinken auf systeme.io-Bezahlseiten (`minikurs1` = 7 € exklusiv am Freebie-Weg, `17minikurs1` = 17 € öffentlich).
...
> **Facebook-Seite verbunden (Stand 2026-06-29):** Katjas **Facebook-Seite** „Katja Jung - Yoga für Frauen 40+" (Seiten-ID `1256512790869846`, `facebook.com/profile.php?id=61591338606353`) ist jetzt als Messenger-Kanal **im selben ManyChat-Pro-Konto** wie Instagram verbunden (`fb5063695`). Crossposting läuft auf die Seite. Damit laufen Auto-DMs **auf Instagram UND Facebook**. Details + Recipe + Gotchas: Memory `project_manychat_facebook_setup`. Follower des alten Profils (214) nicht automatisch übertragbar → organisch + „Umzugs-Post".

- **Account:** @yoga.statt.funktionieren + FB-Seite, **Pro-Testphase** (Pro-Funktionen wie „any post" + öffentliche FB-Antwort verfügbar). Kein MCP/API-Connector → Bedienung über die **Claude-Chrome-Extension** (eingeloggte Session) oder copy-fertige Texte.
- **Flows (Stand 2026-06-23):**
  ...
  - **„Auto-DM 7ZEICHEN (Funktionsmodus-Check)"** — Keyword **7ZEICHEN** → 1:1 wie der CHECK-Flow (Opening-DM → Link-DM `check.katjajung.com`, ohne 7-€/Telegram). **Als DRAFT/Off gebaut 2026-06-23** als distinktives Zweit-Keyword fürs „any post"-Setup (weil „Funktionsmodus" als Themenwort in echten Kommentaren vorkommt). Ab 28.06. auf „any post" + Live; **in neuem Content ab dann 7ZEICHEN statt CHECK** ansagen.
  - **Die drei Drive-Freebie-Flows (3MINUTEN · RESET · NACHTRUHE) haben jetzt alle denselben Aufbau:** Opening-DM (Button „Schick…") → Link-DM mit **3 Buttons in fester Reihenfolge: [Freebie öffnen] · [10-Min-Rückkehr 7 €] · [Zur Telegram-Gruppe]**. Der 7-€-Mini-Produkt-Button (`check.katjajung.com/angebot`) steht jetzt direkt in der DM (Wunsch Katja 2026-06-23), plus ein 7-€-Satz im Liefer-Text. Telegram-Link `t.me/+mJbsYV-tVkc3ZWYy`. **Gotcha gelernt 2026-06-23:** Button-Label max. 20 Zeichen (darum „10-Min-Rückkehr 7 €"); ein End-Emoji frisst alle folgenden Zeilenumbrüche → 💛 an den **Zeilenanfang** setzen.
  - **„Auto-DM 3MINUTEN (Körpercheck)"** — Keyword **3MINUTEN** → Drive-Link `…1G31qFKxQeDdAH8yXirI1Z48a39cCRzROkaScdUaMpLk`. Status **DRAFT/Off**, 7-€-Button am 2026-06-23 ergänzt.
  - **„Auto-DM RESET (5-Minuten-Körper-Reset)"** — Keyword **RESET** → Drive-Link `…1e4bbzXMVQ3ZuNh8GfTPEkFQr9bD93lwCmf18ksXiIoA`. **Als DRAFT/Off gebaut 2026-06-23.**
  - **„Auto-DM NACHTRUHE (Wenn der Kopf nicht ausmacht)"** — Keyword **NACHTRUHE** → Drive-Link `…1EKkAjrlYFzq_cMwmse97t558lXxV9T3-JYyNGXjBNbY`. **Als DRAFT/Off gebaut 2026-06-23.**
  - **STATUS 2026-06-29 — alles live geschaltet:** Auf **Instagram** sind alle 4 Quick-Automations **LIVE auf „any post"** mit deutschen öffentlichen Antworten (7ZEICHEN · 3MINUTEN · RESET · NACHTRUHE); CHECK bleibt post-gebunden live. Auf **Facebook** sind **alle vier LIVE** — 3MINUTEN · RESET · NACHTRUHE · 7ZEICHEN (eigene Flow-Builder-Flows, „any post", DM „As private reply", deutsche öffentliche Antwort); die FB-Variante für **7ZEICHEN (Funktionsmodus-Check) hat Katja selbst fertiggestellt** (Button-Link `check.katjajung.com`, KEIN Drive-Link). „any post" = Pro-Funktion über ALLE Beiträge → distinktive Keywords (7ZEICHEN/3MINUTEN/RESET/NACHTRUHE), CHECK bleibt post-gebunden. **FB-Gotchas (Memory `project_manychat_facebook_setup`):** FB-DM muss „As private reply" sein; öffentliche FB-Antwort erst nach „Refresh Permissions" (`pages_manage_engagement`) möglich; beim Bearbeiten fällt nach Cmd+A+Tippen oft das erste Zeichen weg (URL-„h" prüfen!). **Noch zu tun:** Katja testet jeden FB-Flow (alle 4) mit Zweit-Account-Kommentar (v. a. crossgepostete Reels).
- **Keyword-Konsistenz (jetzt pro Freebie):** Erkennungs-/Funktionsmodus-Content → **7ZEICHEN** (in neuem Content; das alte **CHECK** läuft nur auf seinem bestehenden Reel weiter) · kurzer Übungs-/Spür-Content tagsüber → **3MINUTEN** · Abend-/Feierabend-/Abschalten-Content → **RESET** · Einschlaf-/im-Bett-/„Kopf macht abends nicht aus"-Content → **NACHTRUHE**. Über Pieces/Stories hinweg bewusst abwechseln. Keyword im Post muss exakt zum jeweiligen Trigger passen. Keywords distinktiv halten (siehe Memory `project_manychat_keyword_strategy`). **Gotcha:** Archiviertes/neu hochgeladenes Reel bricht den Trigger („Post not found") → neu zuweisen; Trigger-Schalter muss ON sein.
...
- **Lauf:** `launchd` täglich um 18:00 auf Katjas Mac (`com.katja.instagram-watch.plist`). **PAUSIERT seit 2026-06-29** auf Katjas Wunsch (keine Telegram-Nachrichten mehr gewünscht). **Wichtig:** Bloßes `launchctl unload` reicht NICHT — macOS lädt alles aus `~/Library/LaunchAgents/` beim nächsten Login/Neustart automatisch wieder (genau das passierte: trotz Morgen-Unload kam die 18-Uhr-Nachricht noch). Darum **dauerhaft deaktiviert via `launchctl disable gui/$(id -u)/com.katja.instagram-watch`** (bleibender Sperr-Eintrag, übersteht Neustart) + `bootout`. `.plist` bleibt erhalten. Wieder aktivieren mit `launchctl enable gui/$(id -u)/com.katja.instagram-watch && launchctl load ~/Library/LaunchAgents/com.katja.instagram-watch.plist`.
...
## Hook-Handwerk (gilt für ALLEN Content mit Hook)

**Ein Hook ist ein psychologischer Auslöser, kein schöner Satz** (Katja-Schärfung 2026-06-30). Pflicht bei JEDEM Hook — Reels, Karussells, Stories, Captions: ≥ 2 von 3 Mechanismen (**Neugier-Lücke · Pattern Interrupt · emotionale Erkennung**) + der **Auflöse-Test** („lesen, nicken, weiterscrollen → schwach; weiterschauen müssen → stark"). Konkret schlägt vage, Reiz ganz vorne (~1 Sek), Versprechen halten. **Die 3 gelieferten Hooks (Haupt + 2 Alt) = je eine andere Form**, gelabelt, nie 3× dieselbe Form/derselbe Opener. **Formen-Palette je Content-Typ:** Persönlich (Tür 5) = Confession (mit Zahl) · Widerspruch · Das Unausgesprochene. Nische (Yoga-/Funktionsmodus-Karussells & -Reels) = Widerspruch · Das Unausgesprochene · Erkennung/Spiegel · Insider · Listen-Hook (Confession bleibt persönlich-exklusiv). Volldetail + Beispiele + Katjas virale Referenz-Hooks → `reference/hook-formel.md`, Memory `feedback_starker_hook`.

## Reel-Handwerk: Hook + Watch-Time

Zwei Quelldateien steuern, wie Video-Reels gebaut werden (von den Reel-Skills mitgelesen):

- **Hook** (Einstieg, Scroll-Stop in 1–2 Sek) → `reference/hook-formel.md` (Mechanismen + 3-Formen-System, siehe „Hook-Handwerk" oben).
